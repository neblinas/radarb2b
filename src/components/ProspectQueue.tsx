...
import { ChevronLeft, ChevronRight, Flame, Loader2, Search, Undo2, UserPlus } from "lucide-react";
import { useEffect, useEffectEvent, useState } from "react";
import { supabase } from "@/lib/supabase";

type Prospect = { company_id: string; company_name: string; company_nif: string | null; prospect_score: number; participation_count: number; participation_12m: number; award_count: number; total_award_value: number; last_participation: string | null; cpv_codes: string[] | null; categories: string[] | null; prospect_id: string | null; prospect_status: string | null; assigned_to: string | null; assigned_at: string | null; next_action_at: string | null; total_count: number };
...
export default function ProspectQueue() {
  ...
  const [error, setError] = useState("");
    ...
    useEffect(() => { let active = true; void supabase.auth.getUser().then(({ data }) => { if (!active) return; setCurrentUserId(data.user?.id ?? null); const role = typeof data.user?.app_metadata?.role === "string" ? data.user.app_metadata.role : ""; setCanManageTeam(role === "admin" || role === "commercial_manager"); }); void supabase.rpc("prospect_categories").then(({ data }) => { if (active && Array.isArray(data)) setCategories(data as string[]); }); return () => { active = false; }; }, []);

  const load = useEffectEvent(async () => {
    setLoading(true); setError("");
    const { data, error: rpcError } = await supabase.rpc("prospect_queue", { p_query: query || null, p_min_score: Number(minimumScore) || 0, p_status: status || null, p_assignment: assignment, p_page: page, p_page_size: pageSize, p_category: category || null });
  ...
  useEffect(() => { const timer = window.setTimeout(load, 250); return () => window.clearTimeout(timer); }, [query, minimumScore, status, category, assignment, page, reloadKey]);
  ...
  async function release(prospectId: string) {
    setClaiming(prospectId); setError("");
        const { error: rpcError } = await supabase.rpc("prospect_release", { p_prospect_id: prospectId });
    if (rpcError) setError("Não foi possível remover esta empresa. Confirma que tens permissão para o fazer.");
    setReloadKey((current) => current + 1); setClaiming(null);
  }
