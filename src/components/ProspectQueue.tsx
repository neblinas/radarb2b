...
export default function ProspectQueue() {
  ...
  useEffect(() => { let active = true; void supabase.auth.getUser().then(({ data }) => { if (active) setCurrentUserId(data.user?.id ?? null); }); return () => { active = false; }; }, []);
  ...
  async function release(prospectId: string) {
    ...
    if (rpcError) setError("Não foi possível remover esta empresa da tua lista.");
    setReloadKey((current) => current + 1); setClaiming(null);
  }
