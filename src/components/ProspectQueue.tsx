...
export default function ProspectQueue() {
  ...
  useEffect(() => { let active = true; void supabase.auth.getUser().then(({ data }) => { if (!active) return; setCurrentUserId(data.user?.id ?? null); const role = typeof data.user?.app_metadata?.role === "string" ? data.user.app_metadata.role : ""; setCanManageTeam(role === "admin" || role === "commercial_manager"); }); return () => { active = false; }; }, []);
