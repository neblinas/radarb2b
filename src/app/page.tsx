...
export default function Home() {
      ...
      setKpis((current) =>
        current.map((kpi, index) => ({
          ...
          value: results[index].error
            ? "—"
            : (results[index].count ?? 0).toLocaleString(
                "pt-PT",
              ),
        })),
      );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      const [procedures, companies, entities, cpvs] =
        await Promise.all([
          supabase
            .from("procedures")
          ...
          supabase
            ...
            .or(
              `cpv_code.ilike.${pattern},description.ilike.${pattern}`,
            )
            .limit(5),
        ]);
        ...
        <div className="relative overflow-hidden rounded-[28px] border border-cyan-950/80 bg-[#09182a] p-6 sm:p-8">
          ...
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400 sm:text-base">
              Pesquisa procedimentos, acompanha oportunidades e identifica padrões
              de contratação pública num único espaço.
            </p>
          </div>
                ...
                <div>
                  ...
                  {searchResults.companies.length ? (
                    searchResults.companies.map((item) => (
                      <div
                        key={item.id}
                        className="mb-2 rounded-xl bg-slate-800/60 p-3"
                      >
                        <p className="text-sm font-medium text-white">
                          {item.name || "Sem nome"}
                        </p>
              ...
              return (
                ...
                >
                  <div className="flex h-full flex-col">
                    ...
                    <div className="mt-auto pt-5 text-xs font-semibold text-cyan-400">
                      Abrir área →
                    </div>
                  </div>
                </Link>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-slate-500">
                  Os módulos avançados de empresas, entidades e análises serão
                  adicionados numa fase seguinte. Os dados já podem ser
                  pesquisados diretamente através da pesquisa global.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-slate-400">
                  ...
                  mercado de contratação pública.
                </p>
              </div>
            </div>
          </div>
