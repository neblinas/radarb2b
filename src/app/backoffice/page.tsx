...
import { ArrowRight, BarChart3, Building2, CheckCircle2, Clock3, FileCheck2, LockKeyhole, Users, Zap } from "lucide-react";
...
export default function BackofficePage() {
  ...
  return <BackofficeShell email={email} role={role}><div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-400">Visão geral</p><h1 className="mt-3 text-3xl font-semibold tracking-tight text-white">Operações comerciais</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500">Uma visão única das contas, receita, utilização e atividade do Adjudata.</p></div><div className="rounded-xl border border-emerald-400/20 bg-emerald-400/5 px-4 py-3 text-sm text-emerald-300"><CheckCircle2 className="mr-2 inline" size={16} />Sessão autorizada</div></div><div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
    ...
    ["Autopilot", "Prospeção e vendas automáticas", Zap, "/backoffice/autopilot", "Abrir painel"],
