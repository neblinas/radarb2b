-- Radar B2B — Ponte Stripe -> motor de comissões (service role).
-- Run once in Supabase SQL Editor as project owner.
--
-- O webhook Stripe corre com service role (sem auth.uid()), por isso as RPCs
-- normais (que exigem crm_has_role) não podem ser usadas. Esta função espelha a
-- lógica de comissões sem verificação de role, ficando apenas acessível ao
-- service role (sem grant a authenticated/anon).

-- Processa as comissões de uma subscrição a partir do estado Stripe.
-- p_paid_months: número de mensalidades pagas (mensal). Se null, é inferido
-- pelo número de faturas pagas se disponível, senão por meses decorridos.
create or replace function public.commission_sync_subscription_service(
...
begin
  -- Reutiliza o motor principal, contornando a verificação de role ao
  -- injetar um contexto de role temporário não é possível; por isso o motor
  -- foi refatorado para uma função interna sem verificação e esta é a porta
  -- de serviço. Ver commission_engine_internal abaixo.

  -- Cancela comissões futuras (pending) se a subscrição foi cancelada antes da
  -- 2.ª mensalidade (clawback mensal) ou em qualquer altura para anuais.
  if p_is_cancelled then
    update public.commercial_commission_ledger
      set status = 'cancelled',
          note = coalesce(note || ' | ', '') || 'cancelada por cancelamento da subscrição'
      where organization_id = (
              select organization_id from public.commercial_attributions
              where client_user_id = (select user_id from public.subscriptions where id = p_subscription_id)
              limit 1)
        and subscription_id = p_subscription_id::text
        and status = 'pending';
  end if;

  perform public.commission_engine_internal(p_subscription_id, p_paid_months, p_is_annual);
end;
$$;

-- Motor interno sem verificação de role. NÃO deve ser exposto a authenticated.
...
-- A porta pública com role check passa a delegar no motor interno.
...
begin
  ...
  return query select * from public.commission_engine_internal(p_subscription_id, p_paid_months, p_is_annual);
end;
$$;

-- Recalcula as comissões de todas as subscrições de um cliente (para o webhook).
create or replace function public.commission_resync_client(p_client_user_id uuid)
returns void
language plpgsql security definer set search_path = public as $$
declare
  sub record;
begin
  for sub in select id from public.subscriptions where user_id = p_client_user_id loop
    perform public.commission_engine_internal(sub.id, null, null);
  end loop;
end;
$$;

-- Só o service role deve poder chamar estas funções: NÃO conceder a authenticated.
revoke all on function public.commission_sync_subscription_service(uuid, integer, boolean, boolean) from public;
revoke all on function public.commission_engine_internal(uuid, integer, boolean) from public;
revoke all on function public.commission_resync_client(uuid) from public;
grant execute on function public.commission_sync_subscription_service(uuid, integer, boolean, boolean) to service_role;
grant execute on function public.commission_resync_client(uuid) to service_role;
