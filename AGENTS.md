...
## Público-alvo: produto 100% autónomo (regra transversal)
O Adjudata é, por objetivo de produto, uma plataforma **100% autónoma e self-service**. Não existe contacto telefónico nem venda assistida.

Implicações obrigatórias em qualquer alteração:
- Todo o fluxo deve funcionar de ponta a ponta sem intervenção humana da nossa parte.
- Mensagens, UI, emails e copies NÃO devem prometer ou sugerir reuniões, demonstrações ao vivo, telefonemas, "agendar conversa" ou qualquer contacto assistido.
- CTAs devem encaminhar para a autonomia do utilizador (ex.: criar conta grátis, usar a plataforma), nunca para combinar interação humana.
- Linguagem deve enquadrar-se em comunicação assíncrona (ex.: responder a um email), nunca síncrona/telefónica.

Exceção única e deliberada:
- A **confirmação/aprovação de envios** do Autopilot (fila `pending_approval`) mantém-se human-in-the-loop. É a única parte que exige ação humana, por segurança, e deve ser preservada.
...
- sem alteracoes fora do âmbito da Issue
